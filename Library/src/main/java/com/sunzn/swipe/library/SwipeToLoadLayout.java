...
public class SwipeToLoadLayout extends ViewGroup {
    ...
    private static final int DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION = 1000;
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                mLastY = y;
                mLastX = x;
                boolean moved = Math.abs(yInitDiff) * 0.5f > Math.abs(xInitDiff);
                ...
                if (Math.abs(xInitDiff) * 0.5f > Math.abs(yInitDiff) && Math.abs(xInitDiff) > mTouchSlop) {
                    return false;
                }
...
//                if (Math.abs(xDiff) * 0.5f > Math.abs(yDiff) && Math.abs(xDiff) > mTouchSlop) {
//                    return false;
//                }
    ...
    RefreshCallback mRefreshCallback = new RefreshCallback() {
        ...
        public void onCancel() {
            if (mHeaderView != null && mHeaderView instanceof SwipeTrigger && STATUS.isStatusDefault(mStatus)) {
                mHeaderView.setVisibility(GONE);
                ((SwipeTrigger) mHeaderView).onCancel();
                if (mRefreshListener != null) {
    ...
    LoadMoreCallback mLoadMoreCallback = new LoadMoreCallback() {
        ...
        public void onCancel() {
            if (mFooterView != null && mFooterView instanceof SwipeTrigger) {
                ((SwipeTrigger) mFooterView).onCancel();
                mFooterView.setVisibility(GONE);
            }
        }
